...
export async function fetchDuckDuckGoLite(query) {
    try {
        const searchQuery = encodeURIComponent(`site:amazon.in OR site:flipkart.com ${query}`);
        ...
        $('a.result-link').each((i, el) => {
            ...
            if (title && href && (href.includes('amazon.in') || href.includes('flipkart.com'))) {
        ...
        if (results.length === 0) {
            $('a[href]').each((i, el) => {
                ...
                if (title.length > 10 && (href.includes('amazon.in') || href.includes('flipkart.com'))) {
