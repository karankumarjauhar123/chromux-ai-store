    ...
    try {
        ...
        const processItems = (items, platformName, logoUrl) => {
            return items.map(item => {
                ...
                if (!item.imageUrl || item.imageUrl.length < 10 || item.imageUrl.includes('pixel.png')) {
                    ...
                    item.imageUrl = `https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&q=80`; // Good default image, or category specific
                    // Using modern unsplash categories for fallbacks (source.unsplash.com is deprecated)
                    item.imageUrl = `https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=400&q=80`; // default product
                    if (keyword === 'earbuds+wireless') item.imageUrl = `https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=400&q=80`;
                    else if (keyword === 'smartphone') item.imageUrl = `https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400&q=80`;
                    else if (keyword === 'laptop+computer') item.imageUrl = `https://images.unsplash.com/photo-1496181130204-7552cc14bac4?w=400&q=80`;
                    else if (keyword === 'smartwatch') item.imageUrl = `https://images.unsplash.com/photo-1542496658-e33a6d0d50f6?w=400&q=80`;
                    else if (keyword === 'television+screen') item.imageUrl = `https://images.unsplash.com/photo-1593305841991-05c297ba4575?w=400&q=80`;
                    else if (keyword === 'camera') item.imageUrl = `https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=400&q=80`;
                    else if (keyword === 'shoes+sneakers') item.imageUrl = `https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&q=80`;
                    else if (keyword === 'clothing+fashion') item.imageUrl = `https://images.unsplash.com/photo-1489987707025-afc232f7ea0f?w=400&q=80`;
                    else if (keyword === 'beauty+cosmetics') item.imageUrl = `https://images.unsplash.com/photo-1596462502278-27bfdc403348?w=400&q=80`;
                    else if (keyword === 'bag+backpack') item.imageUrl = `https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=400&q=80`;
                    else if (keyword === 'bluetooth+speaker') item.imageUrl = `https://images.unsplash.com/photo-1608043152269-423dbba4e7e1?w=400&q=80`;
                    else if (keyword === 'gaming+keyboard') item.imageUrl = `https://images.unsplash.com/photo-1618384887929-16ec33fab9ef?w=400&q=80`;
                    else if (keyword === 'tablet+device') item.imageUrl = `https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?w=400&q=80`;
                    else if (keyword === 'charger+powerbank') item.imageUrl = `https://images.unsplash.com/photo-1609592424083-d5d4ad784260?w=400&q=80`;
                    else if (keyword === 'book') item.imageUrl = `https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=400&q=80`;
                    
                    // Add signature for cache busting/variety if wanted
                    item.imageUrl += `&sig=${hash}`;
                }
...
async function fetchPriceBefore(query, platformHint = '') {
    try {
        ...
        $('div.unit').each((i, el) => {
            ...
            // 🖼️ Extract REAL product image from div.unit (handling lazy loading attributes)
            const imgEl = $(el).find('img').first();
            let imageUrl = '';
            if (imgEl.length > 0) {
                const srcAttr = imgEl.attr('src') || '';
                const dataSrcAttr = imgEl.attr('data-src') || '';
                const dataLazySrcAttr = imgEl.attr('data-lazy-src') || '';
                
                if (srcAttr && !srcAttr.includes('pixel.png')) {
                    imageUrl = srcAttr;
                } else {
                    imageUrl = dataSrcAttr || dataLazySrcAttr || srcAttr;
                }
            }
