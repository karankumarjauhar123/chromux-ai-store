...
// ============================================================
// 3-LAYER PARALLEL RACE: Direct + PriceBefore + DDG (100% Free)
// All fire at once — first valid result wins = maximum speed!
// ============================================================
...
async function fetchWithFallback(platform, query) {
    ...
    const allPromises = [
        // Layer 1: Direct (priority — if it works, fastest)
        directFn ? directFn(query).catch(() => []) : Promise.resolve([]),
        // Layer 2: PriceBefore (starts immediately, no waiting)
        fetchPriceBefore(query, platform).catch(() => []),
        // Layer 3: DDG Lite (starts immediately, no waiting)
        fetchDuckDuckGoLite(query, site).catch(() => [])
    ];
    
    // Race: return as soon as ANY layer has valid results
    // But give Layer 1 (Direct) a 2s head start advantage
    try {
        const result = await new Promise((resolve) => {
            let resolved = false;
            const layers = ['Direct', 'PriceBefore', 'DDG'];
            
            // Set overall timeout — give up after 8s
            ...
            allPromises.forEach((promise, i) => {
                promise.then(items => {
                    if (!resolved && items && items.length > 0) {
                        // Layer 1 (Direct) resolves immediately
                        // Layer 2/3 resolve after a tiny delay to prefer Direct
