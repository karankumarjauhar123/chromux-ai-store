        ...
        const processItems = (items, platformName, logoUrl) => {
            return items.map(item => {
                ...
                if (!item.imageUrl || item.imageUrl.length < 5) {
                    // Generate a unique placeholder based on product title
                    const seed = encodeURIComponent((item.title || 'product').substring(0, 30));
                    item.imageUrl = `https://api.dicebear.com/7.x/shapes/png?seed=${seed}&size=200&backgroundColor=1a1a2e`;
                }
    ...
    try {
        ...
        $('div.unit').each((i, el) => {
            ...
            results.push({
                ...
                imageUrl: '', // Will get DiceBear placeholder in processItems
                rating,
                discount,
                snippet: 'PriceBefore'
            });
