...
// Support for Vercel Environment Variables (Comma separated if multiple)
if (process.env.GROQ_KEYS) keys.GROQ_KEYS = process.env.GROQ_KEYS.split(',').map(k => k.trim());
if (process.env.GEMINI_KEYS) keys.GEMINI_KEYS = process.env.GEMINI_KEYS.split(',').map(k => k.trim());
if (process.env.CEREBRAS_KEYS) keys.CEREBRAS_KEYS = process.env.CEREBRAS_KEYS.split(',').map(k => k.trim());
